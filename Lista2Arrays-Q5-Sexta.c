...
buscar  e listar elementos. Declarem a
...
#include<stdio.h>
#define MAX 100
int inserir(int v[], int n, int x); // OK
void listar(int v[ ], int n);  // OK
int inserir_ini(int v[], int n, int x); // OK
int remover(int pos,int  v[ ], int n); // OK
...
int main(){
    ...
    size = inserir_ini(valores, size, 5);
    //size = remover(2, valores, size);
    listar(valores, size);
    return 0;
}
int inserir_ini(int v[], int qtde, int novo){
    if(n < MAX){
